...
class Warns:
    ...
    async def setup(self) -> None:
        ...
        await self.cur.execute('''CREATE TABLE IF NOT EXISTS warnings(user_id INTEGER PRIMARY KEY NOT NULL, guild_id INTEGER, warns INTEGER)''')
    ...
    async def create_acc(self, user_id: int, guild_id: int, warns: int) -> namedtuple[...]:
        ...
        await self.cur.execute('''INSERT INTO warnings(user_id, guild_id, warns) VALUES(?, ?, ?)''', (user_id, guild_id, warns))
        await self.conn.commit()
        return self.named_tuple(user_id, guild_id, warns)
    

    async def update(self, user_id: int, guild_id: int, warns: int):
        await self.cur.execute("UPDATE warnings SET warns = ? WHERE user_id = ? AND guild_id = ?", (warns, user_id, guild_id))
        await self.conn.commit()
